...
Cap4jAjaxConf.prototype.setContentType = function(contentType){
	this.contentType = contentType;
};

Cap4jAjaxConf.prototype.setContentTypeToJson = function(){
	this.contentType = "application/json";
};

Cap4jAjaxConf.prototype.setIsShowSuccMsg = function(showSuccmsg){
	this.isSuccMsg = showSuccmsg;
};

Cap4jAjaxConf.prototype.setIsShowProcessBar = function(isShowProcessBar){
	this.isShowProcessBar = isShowProcessBar;
};

Cap4jAjaxConf.prototype.setBeforeSubmitFunc = function(beforeSubmitFunc){
	this.beforeSubmitFunc = beforeSubmitFunc;
};

Cap4jAjaxConf.prototype.setSuccessFunc = function(successFunc){
	this.successFunc = successFunc;
};

Cap4jAjaxConf.prototype.setErrorFunc = function(errorFunc){
	this.errorFunc = errorFunc;
};

Cap4jAjaxConf.prototype.setIsAsync = function(isAsync){
	this.async = isAsync;
};

Cap4jAjaxConf.prototype.submitForm = function(formId, reqUrl){
	$J.submitForm(formId, reqUrl, this);
};

Cap4jAjaxConf.prototype.postByAjax = function(postData, reqUrl){
...
nui.submitForm = function(formId,reqUrl,ajaxConf){
	...
	if(ajaxConf){
		...
		tempAjaxConf = new Cap4jAjaxConf();
	}
    ...
    if(formData){
    	if(typeof(gridId) != "string"){
    		...
    		for(var index = 0;index < gridId.length;index++){
    			var gridData = $J.formatGridSubmitData(gridId[index]);
    		...
    		var gridData = $J.formatGridSubmitData(gridId);
...
nui.getAjaxSetting = function(ajaxConf){
	...
	ajaxSetting.error = function(data, textStatus, jqXHR){
		...
		if(ajaxConf.processMsg){
			$J.closeProcessBar(ajaxConf.processMsg);
		}
		try{
			var jsonData = $.parseJSON(data.responseText);
			if(jsonData.isBizErr){//业务错误标识
				$J.cap4jError(jsonData.errMsg);
			}else{
				//title, url, width, height, bizParams, callBackFunc, isShowCloseIcon, isAllowResize
				$J.showmodaldialog(message.cap4jui.errorPrompt,"exceptionCtrler/toAjaxExceptionPage.do",null,300,jsonData,null,null,false);
			}
		}catch(err){
			if(data.status == 403){
				$J.cap4jError(message.cap4jui.noRight);
			}else if(data.status == 200){
				//Session超时或当前Session被其他Session逐出！
				top.window.location = $J.formatUrl("login/login.do?error=true");
			}
		}
	};
	ajaxSetting.success = function(data){
		...
		if(ajaxConf.isSuccMsg){
			$J.cap4jInfo(message.cap4jui.success);
		}
	};
...
nui.cap4jError = function(msg, title, callBackFun, params){
...
function topShowProcessBar(){
	return nui.loading(message.cap4jui.wait, message.cap4jui.prompt);
}
function topCloseProcessBar(processBar){
	nui.hideMessageBox(processBar);
}
//跨Iframe提示信息
nui.cap4jConfirm = function(msg, title, callBackFun, params){
...
nui.codeTypeRender = function(e, field, codeTypeData){
	if(typeof(codeTypeData) != "string"){
		for(var index = 0;index < codeTypeData.length;index++){
		 ...
		 return $J.getDictText(codeTypeData, e.row[field]);
	}
};
//打开自定义窗口对话框配置类
function Cap4jWinConfig(title, url, width, height, bizParams, callBackFunc){
	if(url){
    	this.url = $J.formatUrl(url);  //页面地址
	}
    this.title = title;            //标题
    this.iconCls = "";             //标题图标
    this.width = 800;              //宽度
...
Cap4jWinConfig.prototype.setUrl = function(url){
	this.url = $J.formatUrl(url);
};
Cap4jWinConfig.prototype.setTitle = function(title){
	this.title = title;
};
Cap4jWinConfig.prototype.setIconCls = function(iconCls){
	this.iconCls = iconCls;
};
Cap4jWinConfig.prototype.setIsAllowResize = function(allowResize){
	this.allowResize = allowResize;
};
Cap4jWinConfig.prototype.setIsAllowDrag = function(allowDrag){
	this.allowDrag = allowDrag;
};
Cap4jWinConfig.prototype.setIsShowCloseButton = function(showCloseButton){
	this.showCloseButton = showCloseButton;
};
Cap4jWinConfig.prototype.setIsShowMaxButton = function(showMaxButton){
	this.showMaxButton = showMaxButton;
};
Cap4jWinConfig.prototype.setIsShowModal = function(showModal){
	this.showModal = showModal;
};
Cap4jWinConfig.prototype.setIsShowMinButton = function(showMinButton){

    ...
    function initHeadMenu(){
    	try{
    		...
    		for(var i=0; i<menuJson.length; i++){
    			var headMenuItem = {};
    			headMenuItem.id = menuJson[i].id;
    			headMenuItem.text = menuJson[i].text;
    			headMenuItem.iconCls = menuJson[i].iconCls;
    			//headMenuItem.url = menuJson[i].url;
    			headMenuJson.push(headMenuItem);
    		}
    		$J.get("menu1").setData(headMenuJson);
    	}catch(err){
    	}
    }
    
    //初始化左侧菜单栏
    function initLeftMenu(pid){
    	var outlookmenu = $J.get('leftTree');
    	
    	try{
    		var headMenuItem = null;
        	var menuItems = null;
        	if(pid){
        		for(var i=0; i<menuJson.length; i++){
        			headMenuItem = menuJson[i];
        			if(headMenuItem["id"]==pid){
        				menuItems = headMenuItem["children"];
        				break;
        			}
        		}
        	}else{//默认展示第一个菜单频道
        		headMenuItem = menuJson[0];
        		menuItems = headMenuItem["children"];
        	}
        	
        	outlookmenu.load(menuItems);
    	}catch(err){
    	}
        /*var menuItems = [
	        {id:"sql",text:"SQL监控统计",
	        	children:[
	        		{id:"sql_runAvgTime",pid:"sql",staType:"runAvgTime",text:"平均执行时长统计"},
	        		{id:"sql_runtimes",pid:"sql",staType:"runtimes",text:"SQL执行时长统计"},
			        {id:"sql_runcount",pid:"sql",staType:"runcount",text:"SQL执行次数统计"},
			        {id:"sql_running",pid:"sql",staType:"running",text:"当前执行SQL监控"}
	        	]
	        },
	        {id:"springbean",text:"SpringBean监控统计",
	        	children:[
	        		{id:"spring_runAvgTime",pid:"springbean",staType:"runAvgTime",text:"平均执行时长统计"},
	        		{id:"spring_runtimes",pid:"springbean",staType:"runtimes",text:"执行时长统计"},
			        {id:"spring_runcount",pid:"springbean",staType:"runcount",text:"调用次数统计"},
			        {id:"spring_running",pid:"springbean",staType:"running",text:"当前调用监控"}
	        	]
	        }
        ];
        outlookmenu.load(menuItems);
        
        //var node = outlookmenu.getNode("sql_runAvgTime");
        //outlookmenu.selectNode(node);
    	*/
    }
    
	//当选择outlookmemu的节点时触发每个监控页面
    function onItemSelect(e) {
        /*var item = e.item;
        var staType = item.staType;
        if(item.pid == "sql"){
        	iframe.src = "../statistic/sqlStatistic.do?staType="+staType;
        }else if(item.pid == "url"){
        	iframe.src = "../statistic/urlStatistic.do?staType="+staType;
        }*/
        var item = e.item;
        var id = item.id;
        var url = item.url;
...
<body id="panel">
									...
									<tr>
										<td style="padding-right:8px">
											...
											<ul id="skinMenu" class="nui-menu" style="display:none;">
									            ...
									            <li onclick="onSkinChanged(this,'noborder')">noborder</li>
									            <!-- 仿bootstrap风格 -->
									            <li onclick="onSkinChanged(this,'bootstrap')">bootstrap</li>
									            <li onclick="onSkinChanged(this,'jqueryui-smoothness')">jqueryui-smoothness</li>
									            
									            <!-- 基于bootstrap风格自定义颜色风格 -->
									            <li onclick="onSkinChanged(this,'red')">red</li>
									            <li onclick="onSkinChanged(this,'green')">green</li>
									            <li onclick="onSkinChanged(this,'yellow')">yellow</li>
									            <li onclick="onSkinChanged(this,'bluesky')">bluesky</li>
											</ul>
										</td>
										<td style="padding-right:8px">
											<a class="nui-menubutton" iconCls="icon-skin" plain="true" menu="#modeMenu" style="color:#006699;margin-top:0px;_margin-top:-2px;">&nbsp;尺寸</a>
											<ul id="modeMenu" class="nui-menu" style="display:none;">
												<li onclick="onModeChange('')">default</li>
										        <li onclick="onModeChange('medium')">medium</li>
										        <li onclick="onModeChange('large')">large</li>
											</ul>
										</td>
										<td>
	...
	<div class="nui-fit" style="padding-top:5px;">
		<div class="nui-layout" style="width:100%;height:100%;" borderStyle="border:0px solid;">
			<div region="west" title="菜单栏" showHeader="true" showSplitIcon="true" 
				width="230" style="border:1px #aaa solid;">
				<!-- <div id="leftTree" class="nui-outlookmenu" style="height: 75%"
		        	onitemselect="onItemSelect" idField="id" parentField="pid" 
		        	textField="text"  dataField="outlookmenus">
		        </div> -->
		        <div id="leftTree" class="nui-outlookmenu" style="height: 100%"
		        	onitemselect="onItemSelect" idField="id" 
		        	textField="text"  dataField="outlookmenus">
		        </div>
			</div>
